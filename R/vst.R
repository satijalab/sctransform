...
vst <- function(umi,
  ...
  # DEAL WITH Inf theta from glmGamPoi
...
get_model_pars <- function(genes_step1, bin_size, umi, model_str, cells_step1, method, data_step1, theta_given, show_progress) {
  ...
  # package checking
  if (method == "glmGamPoi") {
    glmGamPoi.check <- PackageCheck("glmGamPoi", error = FALSE)
    if (!glmGamPoi.check){
      message(
        "For a more efficient implementation of the sctransform,",
        "\n please install the glmGamPoi package",
        "\n--------------------------------------------",
        "\ninstall.packages('BiocManager')",
        "\nBiocManager::install('glmGamPoi')",
        "\n--------------------------------------------",
        "\nmethod is set to poisson until glmGamPoi is installed"
      )
      method <- "poisson"
    }
  }
                                 ...
                                 FUN = function(j) {
                                   ...
                                   if (method == "glmGamPoi") {
                                     fit <- glmGamPoi::glm_gp(data = y, 
                                                  ...
                                                  col_data = data_step1,
                                                   size_factors = FALSE)
                                     theta <- cbind(1 / fit$overdispersions)
                                     model_pars <- as.matrix(data.frame(theta, fit$Beta))
                                     colnames(model_pars) <- gsub(pattern = "Intercept",
                                                                  replacement = "(Intercept)", 
                                                                  x = colnames(model_pars))
                                     return(model_pars)
                                   }
                                 }
                               )
